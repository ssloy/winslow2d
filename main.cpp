...
int main(int argc, char** argv) {
	...
	for (int iter = 0; iter < 10000; iter++) {
		...
		for (int v = 0; v < m.nverts(); v++) {
				if (boundary_verts[v]) continue;
				std::vector<double> coeff(m.nverts(), 0);

				int hfirst = m.first_halfedge(v);
				int cnt = 0;
				int hcur = hfirst;
				do {
					std::vector<Vec3f> stencil(valency[v], Vec3f(0, 0, 0));

					stencil[0] = Vec3f(-sqrtf(2), sqrtf(2), 0);
										stencil[0] = (m.point(m.to(hfirst)) - m.point(m.from(hfirst))).normalize();
					for (int i = 1; i < valency[v]; i++) {
